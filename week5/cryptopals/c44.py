...
def checkForCommonK(pub, msg1, msg2):
    ...
    if priv1 == priv2 and c43.areValidKeys(pub, priv1) and c43.areValidKeys(pub, priv2):
        return (k, priv1)
    return (None, None)
...
if __name__ == '__main__':
    ...
    hashPriv = c43.hash(hex(priv)[2:].encode('ascii'))
    if hashPriv != expectedHashPriv:
        raise Exception(str(hashPriv) + ' != ' + str(expectedHashPriv))
    for msg in messages:
        (_, s, r, m) = msg
        if not c43.verifySignatureHash(m, (r, s), pub):
